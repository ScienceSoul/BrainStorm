...
void train_dense(brain_storm_net *neural, momentum_optimizer *optimizer) {
    ...
    for (int k=1; k<=n_epochs; k++) {
        ...
        int remainder = 0;
        for (int l=1; l<=neural->dense->train->batch_range((void *)neural, batch_size, &remainder); l++) {
            neural->dense->train->next_batch((void *)neural, features, labels, batch_size, &remainder, false);
            double rt = realtime();
        ...
        fprintf(stdout, "MNIST: training time for epoch {%d} : %f (s).\n", k, train_time);
